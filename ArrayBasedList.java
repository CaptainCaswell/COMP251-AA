...
class ArrayBasedList<T> implements ADTList<T> {
	...
	//Copied code from assignment example for constructing ArrayList but with a specified initial capacity
	public ArrayList(int initialCapacity) { //Time: O(1), Space: O(n)
		capacity = initialCapacity;
		items = (T[]) new Object[capacity];
		size = 0;
	}
	public ArrayList( int capacity ) { //Time: O(1), Space: O(n)
		items = (T[]) new Object[capacity];
		size = 0;
	}
	
	public ArrayList( T[] items ) { //Time: O(n), Space: O(n)
		this.items = (T[]) new Object[capacity];
		this.items = items;
		int size = 0;
		int capacity = 0;
